...
# Filter for SNPs <= 10 Kb away from each other
data <- data %>%
  dplyr::filter(distance <= 10000)

# Create bins
data$bin_dist <- ggplot2::cut_interval(data$distance, length = 100)

# Get left boundary
data$bin <- ggplot2::cut_interval(data$distance, length = 100)
