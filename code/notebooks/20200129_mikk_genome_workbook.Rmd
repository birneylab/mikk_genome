...
# Note this only takes the sequences on the forward strand - still gets 4523 segments 
for i in $(find emfs/segmented/*/*_1.data.txt | head -1 ); do
  # make new directory
  ...
  bsub \
    -M 10000 \
    -o log/20200909_$chr_file\_$bname_short.out \
    -e log/20200909_$chr_file\_$bname_short.err \
    "Rscript --vanilla mikk_genome/code/scripts/20200908_add-hdrr-coords-to-emf-data.R $bname $new_path";
done

# 917 exited with error codes (3606 successful):
grep "Exited with exit code" log/20200909*.out | wc -l
# get error files
grep "Exited with exit code" log/20200909*.out | cut -f1 -d":" | cut -f1 -d"."
```
...
for i in $(find emfs/cleaned/* -type d ); do
  ...
  bsub \
    -M 20000 \
    -o log/20200911_$chr.out \
    -e log/20200911_$chr.err \
...
for i in $(find emfs/consolidated/* | cut -f3 -d"/" | cut -f1 -d"_" | sort | uniq ); do
  bsub \
    -M 20000 \
    -o log/20200911_full_chr_$i.out \
    -e log/20200911_full_chr_$i.err \
...
for i in $( seq 1 24 ); do
  ...
  bsub \
    -M 30000 \
    -o log/20200911_frq_tbl_$i.out \
    -e log/20200911_frq_tbl_$i.err \
...
run_abbababa <- function(data, P1, P2){
  ...
  out_list[["Genome-wide D"]][["Z score"]]
...
O. latipes taxon: <https://www.ebi.ac.uk/ena/browser/view/Taxon:8090>
Contains 8 assemblies for HdrR, HNI and HSOK. Probably already included in the final FASTAs?

```{bash}
cd vcfs
wget ftp://ftp.sra.ebi.ac.uk/vol1/ERZ017/ERZ017166/Hni_Hsok_Kaga_Nilan_SNPs_chr_only.vcf.gz
wget ftp://ftp.sra.ebi.ac.uk/vol1/ERZ017/ERZ017166/Hni_Hsok_Kaga_Nilan_SNPs_chr_only.vcf.gz.tbi

```

Also there's a VCF here: <https://www.ebi.ac.uk/birney-srv/medaka-ref-panel/sequences/>
Aligned to this reference:
```{bash}
wget -P refs/ http://www.ebi.ac.uk/birney-srv/medaka-ref-panel/sequences/oryLat2_corr_unfiltered.fa.gz

```

Need to liftover the VCF. Try `flo`: <https://github.com/wurmlab/flo>
...
# Download old and new FASTAs 
wget http://www.ebi.ac.uk/birney-srv/medaka-ref-panel/sequences/oryLat2_corr_unfiltered.fa.gz
wget ftp://ftp.ensembl.org/pub/release-100/fasta/oryzias_latipes/dna/Oryzias_latipes.ASM223467v1.dna.toplevel.fa.gz

gunzip oryLat2_corr_unfiltered.fa.gz
gunzip Oryzias_latipes.ASM223467v1.dna.toplevel.fa.gz

# Edit .yaml config file
atom flo_opts.yaml 
# Edited lines 16, 17, and 21 to read:
#:source_fa: 'oryLat2_corr_unfiltered.fa'
#:target_fa: 'Oryzias_latipes.ASM223467v1.dna.toplevel.fa'
#:processes: '4'
