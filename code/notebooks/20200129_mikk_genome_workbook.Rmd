...
# Find out what's different in Chr 2

Meeting with Tom:

* Higher repeat content? Ask Jack. Can summarise all the different types of repeats? Herpesvirus repeat?
* Or assembly quality. Can't really get at that, unless there's a measure of mapability per chromosome. 
* Also gene content. Is chr 2 gene-rich or gene-poor? Number of genes or total length of gene content (exons) / total length of chromosome. 

# Try other Simon Martin tutorial on fD
...
# Get gene content per-chromosome

## Gather data from Ensembl

HdrR index: <https://asia.ensembl.org/Oryzias_latipes/Info/Index>
Coding regions fasta page: <ftp://ftp.ensembl.org/pub/release-101/fasta/oryzias_latipes/cds/>
...
## Get sequences to find all unique numbers
...
#### LD of first window by exon coverage
...
### Get median and plot against LD
...
## Total number of SNPs or comparisons? 
...
```
   ...
   35849304 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/10.ld
   33628705 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/11.ld
   20296544 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/12.ld
   31234421 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/13.ld
   24737777 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/14.ld
   24107349 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/15.ld
   33794836 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/16.ld
   32416583 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/17.ld
   88536268 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/18.ld
   31803630 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/19.ld
   54768157 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/1.ld
   25460469 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/20.ld
   29680373 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/21.ld
   17837411 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/22.ld
   20639723 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/23.ld
   26408595 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/24.ld
   83028231 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/2.ld
   35789791 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/3.ld
   26365446 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/4.ld
   22387640 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/5.ld
   22042719 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/6.ld
   34370140 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/7.ld
   34606016 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/8.ld
   34432214 ld/20200727_mikk_maf-0.10_window-50kb_no-missing/9.ld
  824222342 total
...
## Get sequences for reverse complements
...
# Investigate ABBA BABA for 131-1
 
## Split original VCF by individual and sub-populations
...
## Bind subpops and ONT MIKK samples to final
...
### Combine EMF, ONT samples, and sub-popns
...
* All chromosomes when MIKK(P3) -> KW(P2) except 9,10,12,17,20
* 131_1(P3) is a main offender, especially when with MIKK(P2) or KW(P2), but also with all other strains. * 131_1(P3) chr 23 has huge scores (2:12)
...
## Do the same for each of the ONT samples
...
# Run sliding windows ABBA BABA on MIKK
...
## Convert EMF files to `.geno` format and bind to VCF geno

```{bash}
mkdir geno/20201021_final
...
## Make `pop.txt` file *directly on cluster*
...
## Run ABBA BABA in sliding windows
...
# Make karyoplot with SNP distribution
...
## Get SNP density for HdrR, HNI and HSOK
...
# Pull to local
scp brettell@ebi:/hps/research1/birney/users/ian/mikk_paper/geno/20201023_abba_sw_granges_final/20201023_mikk.txt ~/Documents/Data/20201012_mikk_genome
```

```{r}
# Read in file on local
mikk_ranges_df = read.table("~/Documents/Data/20201012_mikk_genome/20201023_mikk.txt",
                            col.names = c("chr", "pos"), 
                            sep = "\t", 
                            as.is = T)
...
# Get ABBA BABA for 131_2, 134_1, and 139_4

### Add to "ONT" samples, then combine with sub-popns and MIKK
...
# Run analysis on new EMF files from Ensembl 101
...
# Get singleton counts per sample
...
## Plot per-chr data
```{r}
sing_df = read.table(here::here("data", "20201107_singletons_per_chr.txt"),
                     sep = "\t",
                     header = T,
                     check.names = F)
...
sing_df %>% 
  ggplot() +
  geom_col(aes(sample, nSingletons, fill = sample)) +
  xlab("Sample") +
  ylab("Number of singleton SNPs") +
  coord_flip() +
  guides(fill = F) +
...
```{r}
# Read in data 
sing_df = read.table(here::here("data", "20201106_full-run_line-ids_singleton_counts.txt"),
                     skip = 1,
                     sep = "\t",
                     comment.char = ",",
                     header = T,
...
# remove weird things in column names
colnames(sing_df) = gsub("\\[.+\\]|#|\ ", "", colnames(sing_df))
...
snp_counts %>% 
  ggplot() +
    geom_col(aes(chr, snps_per_megabase, fill = chr)) +
    guides(fill = F) + 
    xlab("Chromosome") +
    ylab("Non-missing, biallelic SNPs per megabase") +
    theme_bw()
```
```{r}
...
## Combine with exon proportion counts and plot again
...
## Plot SNPs per Mb against prop exons
...
# Get heatmaps for sibling lines

## Make list of sibling lines on cluster
...
## Make new VCF with sib-lines only
...
lapply(high_ld_lst, function(x){
  ...
  png(filename = paste("~/Documents/Docs/medaka pics/20200602_mikk_genome/",
      ...
      units = "cm",
      res = 300)
  heatmap(x[["geno_mat"]], Rowv = NA, Colv = NA)
  dev.off()  
})

```

# Create karyoplot with SV density

## Copy file from Adrien's directory

```{bash}
cp /hps/research1/birney/users/adrien/analyses/medaka_DNA_promethion/brain_run2/DNA_analysis/results/SV/sniffles_all/merged.vcf vcfs/sv_adrien.vcf
```
